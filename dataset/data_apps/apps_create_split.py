...
def create_split(split="train", name="train"):
    ...
    with open('dataset/data_apps/data_split/' + name + ".json", "w") as f:
        json.dump(paths, f)
    return paths
...
paths_to_probs = ["dataset/data_apps/APPS/train", "dataset/data_apps/APPS/test"]
names = ["train", "test"]
...
with open("dataset/data_apps/data_split/train_and_test.json", "w") as f:
